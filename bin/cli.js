...
var y18n__default = /*#__PURE__*/_interopDefaultLegacy(y18n);
var snappy__namespace = /*#__PURE__*/_interopNamespace(snappy);
...
const mainFilename = url.fileURLToPath((typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('cli.js', document.baseURI).href))).split('node_modules')[0];

const __dirname$1 = url.fileURLToPath((typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('cli.js', document.baseURI).href)));
...
var esmPlatformShim = {
  ...
  cliui: cliui__default['default'],
  findUp: escalade__default['default'],
  getEnv: key => {
...
function applyExtends(config, cwd, mergeExtends, _shim) {
  ...
  if (Object.prototype.hasOwnProperty.call(config, 'extends')) {
    ...
    defaultConfig = isPath ? JSON.parse(shim$1.readFileSync(pathToDefault, 'utf8')) : require(config.extends);
...
function Yargs$1(processArgs = [], cwd = shim.process.cwd(), parentRequire) {
  ...
  self.resetOptions = self.reset = function resetOptions(aliases = {}) {
    ...
    validation$1 = validation$1 ? validation$1.reset(localLookup) : validation(self, usage$1, y18n, shim);
    command$1 = command$1 ? command$1.reset() : command(self, usage$1, validation$1, globalMiddleware, shim);
    if (!completion$1) completion$1 = completion(self, usage$1, command$1, shim);
...
function readFileOrStdin(filePath, encoding) {
    return __awaiter(this, void 0, void 0, function* () {
        if (filePath === StdIO.In) {
            return yield readStdinSync(encoding);
        }
        else {
            return Promise.resolve(fs__namespace.readFileSync(filePath, encoding));
        }
    });
}
function readStdinSync(encoding) {
    return new Promise(function readStdinSync(resolve) {
        let input = '';
...
function writeFileOrStdout(filePath, content) {
...
function compress(argv) {
    return __awaiter(this, void 0, void 0, function* () {
        const input = yield readFileOrStdin(argv.input, argv.encoding);
        if (!input) {
            return;
        }
        const payload = snappy__namespace.compressSync(Buffer.from(input, argv.encoding));
        writeFileOrStdout(argv.output, payload);
    });
}
function decompress(argv) {
    return __awaiter(this, void 0, void 0, function* () {
        const input = yield readFileOrStdin(argv.input, Encoding.Hex);
        if (!input) {
            return;
        }
        const payload = snappy__namespace.uncompressSync(Buffer.from(input, Encoding.Hex), { asBuffer: true });
        writeFileOrStdout(argv.output, payload);
    });
}
